...
bool hrm_enable(HRMDevice *dev, HRMFeature features) {
  ...
  if (features & HRMFeature_SpO2) {
    ...
    // Clear the assumed-worn state the SpO2 path leaves behind, so the green path's own ADT is what
    // decides. Otherwise the first HR samples after every SpO2 window report as on-wrist real BPMs
    // (feeding the daily HR logger and the health event) even with the watch off the wrist.
    dev->state->is_wear = false;
  }
#ifdef CONFIG_MFG
