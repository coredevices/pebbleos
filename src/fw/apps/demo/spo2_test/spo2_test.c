...
typedef struct {
  ...
  // True while our RAM-only blood oxygen override is in force.
  bool blood_oxygen_override;
...
static void prv_enable_spo2(AppData *app_data) {
  // Allow the SpO2/IR path for the duration of the test; the HRM manager masks it out entirely
  // when blood oxygen monitoring is off. Use the RAM-only override rather than writing the user's
  // pref: that one is persisted and synced to the phone, so a force-kill or crash here would strand
  // daily SpO2 sampling switched on, with Settings showing a state the user never chose.
  app_data->blood_oxygen_override = true;
  activity_prefs_set_blood_oxygen_override(true);
...
static void prv_disable_spo2(AppData *app_data) {
  ...
  if (app_data->blood_oxygen_override) {
    activity_prefs_set_blood_oxygen_override(false);
    app_data->blood_oxygen_override = false;
  }
}
