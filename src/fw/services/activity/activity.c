...
static void prv_spo2_init(void) {
  ...
  s_activity_state.spo2.hrm_session = hrm_manager_subscribe_with_callback(
      ...
      HRMFeature_SpO2, prv_spo2_subscription_cb, NULL);
...
// us: they share the one optical path, so starting on top of an open daily window would leave the
// attempt starved until the manager's slice hand-off, with too little of its budget left to
// converge. Defer instead and retry on the next tick.
static bool prv_activity_spo2_can_start_attempt(void) {
  return !s_activity_state.hr.currently_sampling &&
         !s_activity_state.spo2.currently_sampling &&
...
static void prv_activity_spo2_init(void) {
  ...
  s_activity_state.activity_spo2.hrm_session = hrm_manager_subscribe_with_callback(
      INSTALL_ID_INVALID, ACTIVITY_HRM_SUBSCRIPTION_OFF_PERIOD_SEC, 0 /*expire_s*/,
      HRMFeature_SpO2, prv_activity_spo2_subscription_cb, NULL);
...
static void NOINLINE prv_process_minute_data_tail(time_t utc_sec) {
  ...
  // Must run with our mutex released: it calls into workout_service, which takes the workout mutex,
  // while workout_service_start_workout() holds that mutex and calls back into us (taking the
  // activity mutex). Holding both here would invert the order and deadlock. The other two callers
  // (the HR and SpO2 data callbacks) are already unlocked, and all three run on KernelBG.
  prv_activity_spo2_update(time_get_uptime_seconds());
#endif
