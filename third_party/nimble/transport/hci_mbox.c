...
void hci_log_to_console(uint8_t type, uint8_t * data, uint16_t len, int host2ctrl)
{
    ...
    *trace++ = type;
    ...
    portDISABLE_INTERRUPTS();
    for (int i=0;i<len;i++)
        uart_write_byte(RAW_UART,temp[i]);
    portENABLE_INTERRUPTS();
#endif    
}
#else
    ...
    case HCI_H4_EVT:
        {
            ...
            if (ev->opcode==BLE_HCI_EVCODE_COMMAND_COMPLETE&&cmd_complete->opcode==0xFC11)
                break;                    
            hci_log_to_console(pkt_type, data, ev->length+sizeof(*ev),0);
        }
        return ble_transport_to_hs_evt(data);
    case HCI_H4_ACL:
        struct os_mbuf *om=(struct os_mbuf *)data;       
        hci_log_to_console(pkt_type, OS_MBUF_DATA(om, uint8_t*), OS_MBUF_PKTLEN(om),0);
...
#if DEBUG==2    
  uart_init(RAW_UART);
  uart_set_baud_rate(RAW_UART, 1000000);
#endif
...
int ble_transport_to_ll_cmd_impl(void *buf) {   
  ...
  hci_log_to_console(HCI_H4_CMD,buf,3+cmd->length,1);
...
int ble_transport_to_ll_acl_impl(struct os_mbuf *om) {
  ...
  hci_log_to_console(HCI_H4_ACL,data,OS_MBUF_PKTLEN(om),1);
